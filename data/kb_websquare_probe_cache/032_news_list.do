...
<header class="fixed">
	<div class="inner-wrap">
		<h1 class="sr-only">새소식</h1>
		<h2 class="page-title" id="pa2">새소식</h2>
		<!-- back버튼 노출시 -->
...
<!-- header End -->
    ...
    $(window).scroll(function() {
		...
		if(_gap <= 56   && !$("body").hasClass('modal-open')){
		   //alert('더보기');
			...
			//AppAlert($('#pageNum').val());
			//AppAlert($('#TOT_PAGE_NUM').val());
			
			if(parseInt($('#pageNum').val()) < parseInt($('#TOT_PAGE_NUM').val())){
				goMoreNewsDet(cateCode);	
			}
		}
	});
    
    function goMoreNewsDet(cateCode){
		...
		$.ajax({
			url : ctx + "/kiwicustomer/getNewsListAjax.do",
			type : "post",
			async : true,
			data : $("#frm").serialize(),
			dataType : 'json',
			success : function( res) {
				...
				if(res.NewNoticeList.length != 0){
					...
					for (var i = 0; i < res.NewNoticeList.length; i++) {
				
						if (res.NewNoticeList[i].NWS_DIV_CD != "4" && res.NewNoticeList[i].NWS_DIV_CD != "6") {
							
							regDttiN = res.NewNoticeList[i].REG_DTTI;
							regDtti = setDateForemat2(regDttiN);
							
							html += '<li>';
							html += '<a href="javascript:getNews('+ res.NewNoticeList[i].NEWNOTICEID + ');">';
							html += '<span class="subject">'+ res.NewNoticeList[i].TITLE + '</span>';
							html += '<p class="date">';
							html += '<em>'+ regDtti + '</em>';
							...
							html += '<span class="new">';
							html += '<span class="sr-only">new</span>';
							html += '</span>';
	...
	function getNewsDet( cateCode) {
		
		//var data = "cateCode="+cateCode;
		var nextpage =	parseInt("0") + 1;
		$("#pageNum").val(nextpage);
		$("#pageSize").val("20");
		$("#cateCode").val(cateCode);
		$.ajax({
			...
			success : function( res) {				
				...
				if(res.NewNoticeList.length != 0){
					...
					var today = new Date();					
					var todayN = textDate2(dateadd(today, -14, "d"));
					var regDttiN = "";
					var regDtti = "";
					var TOT_PAGE_NUM = res.NewNoticeList[0].TOT_PAGE_NUM;
					$('#TOT_PAGE_NUM').val(TOT_PAGE_NUM);
	...
	function getNews(newnoticeID){
		location.href = ctx + '/kiwicustomer/news_detail.do?newnoticeID=' + newnoticeID + '&cateCode=' + $("#cateCode").val();
	}
	// 공지사항
	function goNotice() {
		location.href = ctx + '/kiwicustomer/notice_list.do';
	}
	
	//금융팁
	function goBankTip() {
		location.href = ctx + '/kiwicustomer/bankTip_list.do';
	}
...
<body>
			...
			<div id="contents" class="cus-faq pb36">
				<section class="pd-h">
					<div class="ui-tab">
						<div class="tab-menu type2">
							<ul>
								...
								<li id="tab_news" class="active">
									<button type="button">
										<span>새소식</span>
										<span class="sr-only">선택됨</span>
									</button>
								</li>
								<li onclick="goBankTip()">
									<button type="button">
										<span>금융팁</span>
									</button>
								</li>
					...
					<div class="tab-contents active">
						<!-- 상단 메뉴 -->
						<div class="form-select-group line-type mt28">
							
								
									<span class="form-check" onclick="getNewsDet('1');">
										<input type="radio" name="newnoticeradio" id="'newnoticeradio'+1" checked/>
										<label for="'newnoticeradio'+1">전체</label>
									</span>
								
							
								
									<span class="form-check" onclick="getNewsDet('2');">
										<input type="radio" name="newnoticeradio" id="'newnoticeradio'+2" />
										<label for="'newnoticeradio'+2">수상</label>
									</span>
								
							
								
									<span class="form-check" onclick="getNewsDet('3');">
										<input type="radio" name="newnoticeradio" id="'newnoticeradio'+3" />
										<label for="'newnoticeradio'+3">사회공헌</label>
									</span>
	...
	<form id="frm" method="POST">
		...
		<input type="hidden" id="cateCode"      name="cateCode" />	
		<input type="hidden" id="TOT_PAGE_NUM" 		name="TOT_PAGE_NUM" />
