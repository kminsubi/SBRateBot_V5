...
<header class="fixed">
	<div class="inner-wrap">
		...
		<h2 class="page-title" id="pa2">공지사항</h2>
		<!-- back버튼 노출시 -->
...
<!-- header End -->
	<title data-style="0">공지사항</title>
	<script type="text/javascript">
	var prevPage = "";
	

	function goPage(cateCode, rank){
		location.replace(ctx + '/kiwicustomer/notice_list.do?cateCode=' + cateCode +"&rank="+rank +"&prevPage="+prevPage);
	}
	function getNews(noticeID){
		location.href = ctx + '/kiwicustomer/notice_detail.do?noticeID=' + noticeID;
	}
	// 새소식
	function goNews() {
		location.href = ctx + '/kiwicustomer/news_list.do';
	}
    ...
    $(document).ready( function (){
     ...
     $(window).scroll(function() {
		...
		function goMoreNoticeDet(){
			...
			$.ajax({
				url : ctx + "/kiwicustomer/getNoticeListAjax.do",
				type : "post",
				async : true,
				data : $("#frm").serialize(),
				dataType : 'json',
				success : function( res) {
					...
					if(res.NoticeList.length != 0){
						...
						for (var i = 0; i < res.NoticeList.length; i++) {
							...
							//if (res.NewNoticeList[i].NWS_DIV_CD != "4") {
								
								regDttiN = res.NoticeList[i].REG_DTTI;
								regDtti = setDateForemat2(regDttiN);
								html += '<li>';
								html += '<a href="javascript:getNews('+ res.NoticeList[i].NOTICEID + ');">';
								html += '<span class="subject">'+ res.NoticeList[i].TITLE + '</span>';
								html += '<p class="date">';											
								html += '<em>'+ regDtti + '</em>';							
								if (todayN <= regDttiN) {							
								html += '<span class="new">';
			...
			$.ajax({
				...
				success : function( res) {
					...
					if(res.NoticeList.length != 0){
						...
						var regDttiN = "";
						var regDtti = "";
						var TOT_PAGE_NUM = res.NoticeList[0].TOT_PAGE_NUM;
	...
	<div class="wrapper" id="wrapper">
		...
		<div id="container">
			<div id="contents" class="cus-faq pb36">
				<section id="searchArea" class="pd-h">

					<div class="ui-tab">
						<div class="tab-menu type2">
							<ul>
								<li id="tab_notic" class="active">
									<button type="button">
										<span>공지사항</span>
								...
								<li id="tab_news">
									<button type="button" onclick="goNews();">
										<span>새소식</span>
									</button>
								</li>
