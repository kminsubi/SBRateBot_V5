...
<header class="fixed">
	<div class="inner-wrap">
		<h1 class="sr-only">약관자료실</h1>
		<h2 class="page-title" id="pa2">약관자료실</h2>
		<!-- back버튼 노출시 -->
...
<!-- header End -->
	...
	$(document).ready( function (){
		...
		/* 레이아웃 확인버튼 */
		$("#btnLgPrimary").click( function (){
			UI.layerPopClose('#termsAgree');
			//funcAppTitle_0();
		});
		
		/* 최초 대출 약관 호출 */
		getTmsDet( "20"); //대출 코드 10
		...
		$('#searchAddrBtn').click(function(){
			if($('#keyword').val() != ''){
				...
				/* $('#errMsg').empty();
				$('#errMsg').html("검색주소를 입력 해주세요.");
				modalOpen('#no-text'); */
				UI.alert({
					text: '검색하실 약관을 입력해주세요'
				});
			}
		});
		
//		$("#btnLgFunc1").click(function(){
//			
//		})
	...
	/* PDF ctr */
	function imgViewEvent ( idx, tmsFPath, tmsTtlNm, tmsPageCnt) {
		...
		for ( var i = 0; i<tmsPageCnt; i++) {
			IMGSURL.push("/images_kiwi/tms/"+tmsFPath+"/"+tmsFPath+"_"+tmsPageCnt+"-"+ (i+1) +".png");
			imgs.push(data);
		}
		
		//var pfdUrl= "downPdfFunc('/images_kiwi/tms/"+tmsFPath+"/"+tmsFPath+".pdf')";
		//var pfdUrl= "location.href=('/images_kiwi/tms/"+tmsFPath+"/"+tmsFPath+".pdf')";
		var pfdUrl= '../images_kiwi/tms/'+tmsFPath+'/'+tmsFPath+'.pdf';
		//var pfdUrl= '../images_kiwi/tms/0bfa97bb71cb430a82ab91c1e7efe71e/0bfa97bb71cb430a82ab91c1e7efe71e_2-1.png';
		//$("#btnLgFunc1").attr('onclick', pfdUrl);
		
		$("#btnLgFunc1").attr('href', pfdUrl);
		
		for (var k = 0; k < imgs.length; k++) {
			var tmp = "";
			var img = imgs[k];
            var IMGSURLs = img.IMGSURL;

            for (var j = 0; j < IMGSURLs.length; j++) {
				tmp += '<img src="..' + IMGSURLs[j] + '" />';
            }
		...
		/* 레이어 팝업 옵션 호출 (onClose 로 닫힘 이벤트 확인 가능) */
		UI.layerPop({
			selector : '#termsAgree',
			onClose : function(){
				//funcAppTitle_0();
	...
	function doAjax() {
		...
		E2EWrapper.ajax({
			url : ctx + "/kiwicustomer/CUS500001M.do",
			type : "post",
			async : true,
			data : $("#frm").serialize(),
			dataType : 'json',
               success : function( data) {
				...
				//약관 자료실 상세 리스트
				if ( data.tmsDetList.length != 0) {
					
					var html ='';
					for ( var i = 0; i < data.tmsDetList.length; i++) {
						
						var tmsFileNm = data.tmsDetList[i].TMS_FILE_NM;
						var tmsFilePath = "";
						if(tmsFileNm != undefined){
							tmsFilePath = tmsFileNm.replace(".pdf","");
						}
						var tmsPageCnt = data.tmsDetList[i].TMS_PAGE_CNT;
						var tmsTtlNm = data.tmsDetList[i].TMS_TTL_NM;
						
						html += ''
						+ '<li>'
						+ '	<button onclick="imgViewEvent(\''+ i +'\',\''+ tmsFilePath +'\',\''+ tmsTtlNm +'\',\''+ tmsPageCnt +'\');">'
						+ '	<span>'+ tmsTtlNm +'</span>'
						+ '	</button>'
						+ '</li>';
					}
	...
	function getTmsDet( tmsDivCd){
		...
		if(tms_seq == "2210"){
			$('#KEYWORD').val("계좌통합관리서비스 이용약관");
			data = "TMS_DIV_CD=20&KEYWORD="+$('#KEYWORD').val();
		}else if(tms_seq == "1104"){
			$('#KEYWORD').val("본인계좌 일괄지급정지 대고객 안내사항");
			data = "TMS_DIV_CD=90&KEYWORD="+$('#KEYWORD').val();
		}else if(tms_seq == "1081"){
			$('#KEYWORD').val("본인계좌 일괄지급정지 서비스");
			data = "TMS_DIV_CD=90&KEYWORD="+$('#KEYWORD').val();
		}else{
			data = "TMS_DIV_CD="+tmsDivCd
		}

		$("#TMS_DIV_CD").val(tmsDivCd);
		$.ajax({
			url : ctx + "/kiwicustomer/CUS500001M.do",
			type : "post",
			async : true,
			...
			success : function( data) {
				...
				if ( data.tmsDetList.length != 0) {
					...
					var center_cnt = "";
					var center_tmsFilePath = "";
					var center_tmsTtlNm = "";
					var center_tmsPageCnt = "";
					...
					for ( var i = 0; i < data.tmsDetList.length; i++) {
						...
						if(i == 0 && (tms_seq == "2210" || tms_seq == "1104" || tms_seq == "1081")){
							center_cnt = i;
							center_tmsFilePath = tmsFilePath;
							center_tmsTtlNm = tmsTtlNm;
							center_tmsPageCnt = tmsPageCnt;
						}
					}
				} else {
					html = '등록된 약관이 없습니다.';
				...
				if(tms_seq == "2210" || tms_seq == "1104" || tms_seq == "1081"){
					imgViewEvent ( center_cnt, center_tmsFilePath, center_tmsTtlNm, center_tmsPageCnt);
				}
...
<body>
<form id="frm">
 <input type="hidden" class="wFull" id="TMS_DIV_CD" name="TMS_DIV_CD" value="" />
 <input type="hidden" class="wFull" id="KEYWORD"    name="KEYWORD" value="" /> 	
</form>
		...
		<div id="container">
			<div id="contents">
				<section class="pd-h">
					...
					<div class="ui-tab">
						<div class="tab-menu type2">
							<ul>
										...
										<button type="button" onclick="getTmsDet('10');">
											<span>대출약관</span>
										...
										<button type="button" onclick="getTmsDet('20');">
											<span>예금약관</span>
										...
										<button type="button" onclick="getTmsDet('90');">
											<span>기타약관</span>
										...
										<button type="button" onclick="getTmsDet('30');">
											<span>오픈뱅킹약관</span>
					...
					<div class="pd-all addr" id="showAddrSearchDiv">
						<fieldset class="search-form" id="showAddrSearch1">
							<legend>검색</legend>
							<div class="input-wrap">
								<input type="text" id="keyword" name="keyword" class="form-control" title="약관검색">
							</div>
							<span class="placeholder">검색하실 약관을 입력해주세요</span>
							<button type="button" class="btn-search" id="searchAddrBtn">
	...
	<!-- 레이어 팝업 -->
	<section id="termsAgree" class="layer fullpage" >
		<div class="layer-header">
			<h1 class="layer-title">이용약관</h1>
			<button type="button" class="layer-close cancel">
				<span class="sr-only">레이어 닫기</span>
			</button>
		</div>
		...
		<div class="layer-btns">
			...
			<a type="button" id="btnLgFunc1" class="btn-lg-func1" download>다운로드 </a>
			<button type="button" id="btnLgPrimary" class="btn-lg-primary">확인</button>
		</div>
	</section>
	<!-- //레이어 팝업 -->
