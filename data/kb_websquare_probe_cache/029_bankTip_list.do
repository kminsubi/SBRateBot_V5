...
<header class="fixed">
	<div class="inner-wrap">
		<h1 class="sr-only">금융팁</h1>
		<h2 class="page-title" id="pa2">금융팁</h2>
		<!-- back버튼 노출시 -->
...
<body>
	...
	function getBankTip(bankTipID){
		location.href = ctx + '/kiwicustomer/bankTip_detail.do?newnoticeID=' + bankTipID;
	}
	...
	function getBankTipDet() {
		var nextpage = parseInt("0") + 1;
		$("#pageNum").val(nextpage);
		$("#pageSize").val("20");

		$.ajax({
			url : ctx + "/kiwicustomer/getBankTipListAjax.do",
			type : "post",
			async : true,
			data : $("#frm").serialize(),
			dataType : 'json',
			success : function(res) {
				...
				if( res.bankTipList.length != 0 ) {
					var today = new Date();
					var todayN = textDate2(dateadd(today, -14, "d"));
					var regDttiN = "";
					var regDtti = "";
					var TOT_PAGE_NUM = res.bankTipList[0].TOT_PAGE_NUM;
					$('#TOT_PAGE_NUM').val(TOT_PAGE_NUM);

					for( var i = 0; i < res.bankTipList.length; i++ ) {
						regDttiN = res.bankTipList[i].REG_DTTI;
						regDtti = setDateForemat2(regDttiN);
						html += '<li>';
						html += '<a href="javascript:getBankTip('+ res.bankTipList[i].NEWNOTICEID + ');">';
						html += '<span class="subject">'+ res.bankTipList[i].TITLE + '</span>';
