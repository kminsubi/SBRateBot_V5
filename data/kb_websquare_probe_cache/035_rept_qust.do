...
<header class="fixed">
	<div class="inner-wrap">
		<h1 class="sr-only">자주하는 질문</h1>
		<h2 class="page-title" id="pa2">자주하는 질문</h2>
		<!-- back버튼 노출시 -->
...
<script type="text/javascript">
function plus(){
	var $accordion= $("#search_list");

	var $listAll = $('dd.acc-cont');
	var $button = $('.acc-title button');
	
	$button.on('click' , function(){
		var $list = $("#search_list").find('dl');
...
function getFaqDet(comCdNm){ //카테고리
	
	$('.form-check-group span input').each(function(index, item) {
		$(item).prop("checked",false);
	});
	
	$('input:radio[name='+comCdNm+']').prop("checked",true);
	
	var data = "FAQ_DIV_CD="+comCdNm;
	$.ajax({
		url : ctx + "/kiwicustomer/getFaqDetAjax.do",
		type : "post",
		async : true,
		data : data,
		dataType : 'json',
		success : function(sServerResult) {
			$("#search_list").empty();
			$('#no_answer').hide();
			
			if(sServerResult.faqDetList.length != 0){
				for (var i = 0; i < sServerResult.faqDetList.length; i++) {
					var html = "";
					html += '<dl><dt class="acc-title"><button type="button"><span class="title">';
					html += sServerResult.faqDetList[i].FAQ_TTL_NM;
					html += '<span class="sr-only">더보기</span></button></dt>';
					html += '<dd class="acc-cont"><div class="font-14">'+sServerResult.faqDetList[i].FAQ_CNTS;
					html += '</div></dd></dl>';
...
function searchFaq(comCdNm){
	...
	$('#FAQ_SEARCH_WORD').val($('#search_name').val());
	//var data = "FAQ_SEARCH_WORD="+$('#search_name').val();
	$.ajax({
		url : ctx + "/kiwicustomer/searchFaqListAjax.do",
		type : "post",
		async : true,
		data : $("#frm").serialize(),
		dataType : 'json',
		success : function(sServerResult) {
			...
			if(sServerResult.faqSearchList.length == 0){
				$('#result_name').html($('#search_name').val());
				$('#category').hide();
				$('#no_answer').show();
				//검색 결과 없다고 띄어줘야 함
				...
				//유효한 카테고리 코드 추출				
				var codeList = [];
				$.each(sServerResult.faqSearchList, function(i,value) {
					if(codeList.indexOf(value.FAQ_DIV_CD) == -1 ) codeList.push(value.FAQ_DIV_CD);
				});
				...
				for (var i = 0; i < sServerResult.faqSearchList.length; i++) {
					var html = "";
					html += '<dl><dt class="acc-title"><button type="button"><span class="title">';
					html += replaceAll(sServerResult.faqSearchList[i].FAQ_TTL_NM,$('#search_name').val(),'<b>'+$('#search_name').val()+'</b>');
					html += '<span class="sr-only">더보기</span></button></dt>';
					html += '<dd class="acc-cont"><div class="font-14">'+replaceAll(sServerResult.faqSearchList[i].FAQ_CNTS,$('#search_name').val(),'<b>'+$('#search_name').val()+'</b>');
...
$(document).ready(function(){
	...
	$('#search_name').bind('keypress', function(e){
		if(e.keyCode ==13){
			if($(this).val() != ""){
				$('#search').focus();
				searchFaq();
	...
	$('#search_name').on('blur',function(){
		if($('#search_name').val() == ""){
			var c = $('#search_name').closest('div').attr('class');
			if(c.indexOf('focus') < 0){
				$('#search_label').html('자주 묻는 질문을 검색해 보세요.');
			}
		} else {
			$('#search_label').html('');	
		}			
	...
	if ($("#searchname").val() != "") {			
		$('#search_label').html('');			
		$('#search_btn').focus();			
		$('#search_name').val($("#searchname").val());				
		$("#searchname").val('');			
		$('#search_btn').click();			
	}else{
		//디폴트 예적금
...
<input type="hidden" id="searchname"	name="searchname"	value="" />
	<div class="wrapper" id="wrapper">
		<!-- container -->
		<div id="container">
			<div id="contents" class="cus-faq pb36">
				<!-- 클래스 조합하여 사용 -->
				<section class="pd-h">
					<fieldset class="search-form">
						<legend>검색</legend>
						<div class="input-wrap form-control-group">
							<input type="text" id="search_name" class="form-control" title="검색어 입력" maxlength="50" onblur="inputLengthCheck(this)"
							placeholder="자주 묻는 질문을 검색해 보세요.">
						</div>
<!--
						<span class="placeholder">자주 묻는 질문을 검색해 보세요.</span>
-->
					...
					<div class="ui-toggle faq-category-wrap">
						<button type="button" class="btn-toggle btn-category">
							<span>질문 카테고리</span>
						</button>
						<!-- toggle 영역 -->
						<div class="toggle-contents" style="display: block;">
							
								<div class="form-check-group col3">
								
										<span class="form-check">
											<input type="radio" id="11" name="11" onclick="getFaqDet('11');">
						                    <label for="11">예적금</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="12" name="12" onclick="getFaqDet('12');">
						                    <label for="12">대출</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="13" name="13" onclick="getFaqDet('13');">
						                    <label for="13">앱서비스</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="14" name="14" onclick="getFaqDet('14');">
						                    <label for="14">이체</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="1" name="1" onclick="getFaqDet('1');">
						                    <label for="1">안심이체</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="15" name="15" onclick="getFaqDet('15');">
						                    <label for="15">보안.인증</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="3" name="3" onclick="getFaqDet('3');">
						                    <label for="3">카드서비스</label>
							            </span>							
								
										<span class="form-check">
											<input type="radio" id="17" name="17" onclick="getFaqDet('17');">
						                    <label for="17">기타</label>
							            </span>							
					...
					<div id="no_answer" class="cus-faq" style="display:none">
					<!-- 클래스 조합하여 사용 -->
					<section class="pd-h">
						<div class="no-data" style="height:364px">
							<div>
								<i class="ico"></i>
								<p class="text text-gray8">
									<span class="" id="result_name"></span>에 대한 검색결과가 없습니다. <br>
									다른 검색어로 검색해주세요.
								</p>
							</div>
						</div>
						
						<div class="bg-gray-box round-8 mt28">
							<ul class="list-bullet-dot color-gray">
								<!-- li>원입력하신 단어의 철자가 정확한지 확인해주세요.</li-->
								<li>입력하신 단어의 절차가 정확한지 확인해주세요</li>
                            	<li>%$!#@같은 특수문자를 제외하고 검색해주세요</li>
							</ul>
						</div>
						
					</section>
				</div>
					<div class="ui-accordion" id="search_list"></div>
				</section>
	...
	<form id="frm" method="POST">
		<input type="hidden" id="FAQ_SEARCH_WORD" 		name="FAQ_SEARCH_WORD" />		
	</form>
</body>
