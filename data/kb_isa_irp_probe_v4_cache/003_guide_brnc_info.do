...
<header class="fixed">
	<div class="inner-wrap">
		<h1 class="sr-only">영업점안내</h1>
		<h2 class="page-title" id="pa2">영업점안내</h2>
		<!-- back버튼 노출시 -->
...
<!-- header End -->
	...
	<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=0a0ddf212398bbfa7fe3aca1d383c28a"></script>
	<script type="text/javascript">
	
	$(document).ready( function (){
		
		//화면 진입시 최초 노출
		setValue ("본점", "서울시 송파구 송파대로 260", "02) 2146-8200", "37.4929894", "127.1189598");
		
		/* 셀렉트 레이어 닫기 클릭시 콜백 함수 */
		$('#uiSelect').on('close', function() {
			//이벤트 주고 싶으면 콜백 함수 사용하면 됨
			
			console.log("+++++++++++++++++++++ close");
			
		});

		/* 셀렉트 레이어 확인 클릭시 콜백 함수 */
		$('#uiSelect').on('confirm', function() {
			
			console.log("+++++++++++++++++++++ confirm");
			
			var branchListDet = $("#branchListDet").val();
			var strBranchList = branchListDet.split("|");
			
			setValue (strBranchList[0], strBranchList[1], strBranchList[2], strBranchList[3], strBranchList[4]);
		});
	...
	//영업점 주소 영역 노출
	function setValue (brchNm, brchAddr, brchTelNo, xLocNo, yLocNo) {
		var html = '';
		html += '<small>'+brchNm+'</small>'+
					'<p class="text mb10">'+brchAddr+'<br>'+
					'	<a href="tel:'+funcCallReplace(brchTelNo)+'" class="btn-link"><span>'+brchTelNo+'</span></a>'+
					'</p>';
		$("#brchAddr").html( html);
		
		kakao_map( xLocNo, yLocNo);
	}
	 ...
	 * ex ) 02) 1234-1231 ~3
	 * */
	function funcCallReplace( val) {
		var num = val;
		var telsp = num.split("~");
		var telNum = telsp[0].replace(/[^0-9]/g,"");
		...
		<div id="container">
			<div id="contents">
				<section>
					...
					<div class="bg-gray map-area">
						<span id="kakaoMap"></span>
					</div>

					<div id="brchAddr" class="pd-h pd-t">
						
					</div>

					<div class="page-btns fixed">
						<div id="uiSelect" class="ui-select">							
							<button type="button" id="infoBtn" class="btn-lg-primary">다른 영업점 찾기</button>
							<select id="branchListDet" title="영업점 선택">
							
								<option value="본점|서울시 송파구 송파대로 260|02) 2146-8200|37.4929894|127.1189598" >본점</option>
							
								<option value="분당지점|경기도 성남시 분당구 야탑로 81번길 10 아미고타워 703호|031) 724-6400|37.4108929|127.1282004" >분당지점</option>
